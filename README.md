...
<head>
  ...
  <title>RAVYN Esports — Player Registration</title>
  <style>
    :root {
      /* RAVYN brand — Purple & Gold (matches logo) */
      --bg:#1c0f3a;            /* deep royal purple */
      --card:#120a2a;          /* darker panel purple */
      --muted:#b9b4cf;         /* soft lavender gray */
      --text:#f3f4ff;          /* near-white */
      --brand:#f5b301;         /* signature gold */
      --accent:#ffd34d;        /* light gold accent */
      --danger:#ff5c73;        /* leave as-is */
      --border:#2a174f;        /* subtle purple border */
      --chip:#1a1240;          /* chip/bg elements */
      --btn:#20104a;           /* button surface */
    }
    *{box-sizing:border-box}
    html,body{margin:0;height:100%;color:var(--text);background:radial-gradient(1200px 1000px at 12% -10%,rgba(245,179,1,.15),transparent 60%),var(--bg);font:16px/1.5 system-ui, -apple-system, Segoe UI, Roboto, Inter, "Noto Sans", Arial} 
    ...
    header .tag{font-size:14px;color:var(--muted)}
    .lead{font-size:14px;color:var(--muted)}
    .grid-2{display:grid;grid-template-columns:1.1fr .9fr;gap:18px}
    @media (max-width:900px){.grid-2{grid-template-columns:1fr}}.hero{border:1px solid var(--border);background:linear-gradient(180deg,rgba(245,179,1,.10),transparent 55%),var(--card);padding:22px;border-radius:20px}
.hero h2{margin:8px 0 0;font-size:18px;color:#fff}
.slogan{margin:6px 0 0;color:var(--muted);font-size:14px}

.card{border:1px solid var(--border);background:var(--card);border-radius:20px;padding:18px}
.pill{display:inline-flex;gap:6px;align-items:center;background:var(--chip);border:1px solid var(--border);padding:6px 10px;border-radius:999px;font-size:12px;color:#cbd5e1}

.feature-row{display:flex;flex-wrap:wrap;gap:8px;margin:10px 0 6px}
.feature{flex:1 1 180px;border:1px solid var(--border);background:var(--chip);border-radius:12px;padding:10px 12px;font-size:13px;color:#d6dcef}

.kicker{font-weight:600;margin:8px 0 4px}

.divider{height:1px;background:linear-gradient(90deg,transparent,rgba(124,92,255,.5),transparent);margin:14px 0}

.socials{display:flex;flex-wrap:wrap;gap:10px}
.btn{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--border);background:var(--btn);padding:10px 12px;border-radius:12px;font-size:14px}

form .grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}
form label{display:block;font-size:13px;color:#c6d0de;margin-bottom:6px}
input,select,textarea{width:100%;padding:10px 12px;border-radius:12px;border:1px solid var(--border);background:#0e1217;color:#e9eef5}
textarea{min-height:100px;resize:vertical}
.row-2{grid-column:1 / -1}
.hint{font-size:12px;color:var(--muted);margin-top:4px}
.note{border:1px dashed var(--border);background:#0b1016;border-radius:12px;padding:8px 10px;margin:6px 0}
.preview{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.pholder{border:1px dashed var(--border);background:#0b1016;border-radius:12px;min-height:140px;display:flex;align-items:center;justify-content:center;color:var(--muted);text-align:center;padding:8px}
.progress{height:8px;background:#0f141a;border-radius:999px;border:1px solid var(--border);overflow:hidden}
  ...
  <section class="card" aria-labelledby="reg-title">
    ...
    <form id="regForm" action="https://formspree.io/f/xjkebwkl" method="POST" novalidate>
      ...
      <div class="grid">
        ...
        <div id="igl-wrap" class="row-2" style="display:none">
...
<footer>© 2025 RAVYN Esports. All Rights Reserved. <span id="saveNote" aria-live="polite"></span></footer>
    ...
    function makeLinkChip(url, label){
